...
function noOfStudentsPassedTheExam() {
    ...
    for (var i = 0; i < employeeArray.length; i++) {
        if (employeeArray[i].employeeFail == 0)
            count++;
    }
...
function noOfStudentsPaseedIndividulaSubjects() {
    for (var i = 1; i <= headingsArray.length - 2; i++) {
        ...
        const div = document.createElement('div');
        div.className = 'box';
        div.innerHTML = `<p class="key">Students passed ${i} subject</p><p class="value">${count}</p>`;
        result.appendChild(div);
        reportArray.push(count);
    }
}
...
function prepareResultTable() {
    ...
    for (var i = 0; i < reportArray.length; i++) {
        ...
        if (i == 0) {
            ...
            tableRows = tableRows + `<tr class="reportRow"><th></th><th>No of students passed ${i-3} subject </th><th>${reportArray[i]}</th></tr>`;
